...
class TokenAgentAttention(nn.Module):
    ...
    def __init__(self, dim, num_heads=8, qkv_bias=False, attn_drop=0., proj_drop=0.,
                 agent_num=49, window=14, sparse_ratio=0.5, aggr_ratio=0.4, dim_ratio=0.2):
        super().__init__()
        self.dim = dim
        ...
        # Module Token Adaption để sinh agent tokens
    ...
    def forward(self, x):
        ...
        # --- Tạo agent tokens bằng Token Adaption thay cho pooling ---
        # Bỏ cls token, chỉ lấy image tokens
        q_img = q[:, 1:, :]  # (b, h*w, c)
        # Dùng norm của Q làm attention score đơn giản (có thể thay bằng score khác)
        attention_score = q_img.norm(dim=-1)  # (b, h*w)
        # Dùng cùng attention cho cả x và y để đơn giản hóa
        agent_tokens, _ = self.token_adaption(q_img, attention_score, attention_score)
